...
struct ThenClosure {
    ...
    template <typename PredecessorRef>
    using node_t = decltype(std::declval<const PredecessorRef&>()
                                .then(std::declval<node_props_t&&>(), std::declval<Functor&&>()));

    //! @warning Unconditionally **not** @c noexcept because adding a node may throw.
    template <bool PredecessorIsRoot, typename PredecessorRef>
    auto add_node(const PredecessorRef& predecessor) && noexcept(false) -> node_t<PredecessorRef> {
        auto node = predecessor.then(std::move(node_props), std::forward<Functor>(functor));
#if defined(KOKKOS_EXECUTION_ENABLE_DEBUG_LOGGING)
        PLOG_INFO << "Adding 'then' node " << get_node_ptr(node) << " to graph " << get_graph_impl_ptr(node)
...
struct TransformSenderFor<stdexec::then_t> {
    ...
    auto operator()(const Env& env, stdexec::then_t, Functor&& functor, Sndr&& sndr) const
        ...
        if constexpr (graph_completing_sender<Sndr, Env>) {
            ...
            static_assert(std::same_as<typename decltype(schd)::execution_space, Impl::exec_of_t<Sndr, Env>>);
