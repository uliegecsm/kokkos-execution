...
TEST(get_start_scheduler, sync_wait) {
    ...
    auto sndr_read_env_start_sch = stdexec::read_env(stdexec::get_start_scheduler) | stdexec::let_value([&](auto schd) {
                                       static_assert(std::same_as<decltype(schd), stdexec::run_loop::scheduler>);
                                       return stdexec::schedule(schd) | THEN_STORE_THREAD_ID(&tid);
                                   });

    stdexec::sync_wait(std::move(sndr_read_env_start_sch)); // NOLINT(performance-move-const-arg)

    ASSERT_EQ(tid, std::this_thread::get_id());

    /// The same assertion holds for the delegation scheduler. In fact, in the implementation of @c stdexec::sync_wait
    /// the receiver environment has an overload of the query function that returns a @c run_loop scheduler on the thread
    /// on which it is started for each one of the @c stdexec::get_start_scheduler, @c stdexec::get_delegation_scheduler
    /// and @c stdexec::get_scheduler queries.
    /// See also:
    ///  * https://github.com/NVIDIA/stdexec/blob/ead186b1d8db3ebe37a946ff84a6ce08bf795153/include/stdexec/__detail/__sync_wait.hpp#L62-L67
    auto sndr_read_env_delegation_sch = stdexec::read_env(stdexec::get_delegation_scheduler)
                                      | stdexec::let_value([&](auto schd) {
                                            static_assert(std::same_as<decltype(schd), stdexec::run_loop::scheduler>);
                                            return stdexec::schedule(schd) | THEN_STORE_THREAD_ID(&tid);
                                        });

    stdexec::sync_wait(std::move(sndr_read_env_delegation_sch)); // NOLINT(performance-move-const-arg)
