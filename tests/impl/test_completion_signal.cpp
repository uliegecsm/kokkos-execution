...
template <Kokkos::ExecutionSpace Exec, bool SubmittedReceiver>
struct Receiver {
    using receiver_concept =
        std::conditional_t<SubmittedReceiver, Kokkos::Execution::Impl::SubmittedReceiverTag, stdexec::receiver_tag>;
...
TEST_F(CompletionSignalTest, depend_on_event_policy) {
    ...
    static_assert(Kokkos::Execution::Impl::supports_submitted<Receiver<TEST_EXECUTION_SPACE, true>>);
    static_assert(std::same_as<
                  typename decltype(op_state)::completion_signal_policy_t,
                  Kokkos::Execution::Impl::SubmittedPolicy::DependOnEvent
    >);
