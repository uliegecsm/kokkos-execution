...
/**
 * @test Check that the uncustomized sender returned by @ref Kokkos::Execution::parallel_for
 *       models the @ref Kokkos::Execution::Impl::dispatching_sender concept.
 */
consteval bool test_dispatching_sndr() {
    //! Schedule sender.
    using schd_sndr_t = typename ParallelForTest::schedule_sender_t;
...
static_assert(test_dispatching_sndr());
